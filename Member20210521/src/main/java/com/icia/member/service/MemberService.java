...
public class MemberService {
	...
	public ModelAndView insertDB(MemberDTO dto) {
	...
	public ModelAndView memberLogin(MemberDTO dto) {
		 ...
		 */
		mav = new ModelAndView();
		String loginId = mdao.memberLogin(dto);
		
		// loginId에 값이 있다면 아이디, 비번이 모두 맞았다는 것이고(로그인 성공 처리),
		// loginId에 값이 없다면 틀렸다는 것(로그인 실패 처리)
		
		// 사용자가 로그인을 하고 나면 로그아웃 또는 브라우저를 다딕 전까지는 로그인을 유지하고 있어야 한다.
...
//		MemberDTO memberUpdate = mdao.memberView(loginId);
	...
	public ModelAndView updateProcess(MemberDTO dto) {
		mav = new ModelAndView();
		int updateResult = mdao.updateProcess(dto);
...
//		mav.setViewName("memberList");
	...
	public String idCheck(String mId) {
		 ...
		 * checkresult에 값이 없다면 아이디가 DB에 존재하지 않는다는 것.(사용가능)
		 */
		String result = "";
		if(checkResult == null) {
			result = "ok";
